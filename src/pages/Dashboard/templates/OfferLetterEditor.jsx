...
import OfferLetterFormSidebar from './OfferLetterFormSidebar';
...
import { useOfferLetterDocument } from '../../../hooks/useOfferLetterDocument';
import { generateDocumentPdf } from '../../../utils/pdfGenerator';
import { useMediaQuery } from '../../../hooks/useMediaQuery';
import { generateOfferLetterHtml } from '../../../utils/offerLetterUtils';
...
const OfferLetterEditor = () => {
    ...
    const generateSmartName = () => {
        const candidate = formData.candidateName?.trim();
        if (candidate) {
            setDocumentName(`Offer Letter - ${candidate}`);
            setNameError(null);
            setNameSuggestion(null);
            toast.success("Generated smart name!");
        } else {
            toast.error("Enter 'Candidate Name' in the form first.");
        }
    };
    ...
    const onSave = async () => {
        ...
        if (!trimmedName || ['Untitled Offer Letter'].includes(trimmedName)) {
    ...
    const handleDownloadVersion = async (version) => {
        ...
        try {
            ...
            const fullHtml = generateOfferLetterHtml(versionFormData, versionDocContent, `${documentName} - v${version.version_number}`, [], logoBase64);
        ...
        <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
            <div className="flex flex-col h-screen bg-slate-100 overflow-hidden">
                <SignatureConfigModal
                    ...
                    parties={{
                        disclosingPartyName: formData.employerName,
                        receivingPartyName: formData.candidateName,
                        disclosingSignatoryName: formData.employerSignatoryName,
                        receivingSignatoryName: formData.candidateName, // Candidate signs for themselves
                    }}
                />
                ...
                <SendDocumentModal
                    ...
                    getHtmlContent={async () => {
                        ...
                        return generateOfferLetterHtml(formData, docContent, documentName, signatures, logoBase64);
                    }}
                    signatures={signatures}
                />
